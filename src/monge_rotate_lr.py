...
import distance_utils as dist_utils
...
def gkms_lr(A, B, Q_init, gamma=50.0, max_iter=100, tol=1e-9, min_iter=25):
    ...
    def step(Q):
        ...
        Qn = Qn / jnp.clip(jnp.sum(Qn, axis=1, keepdims=True), 1e-18) / n # Diagonal projection of the positive kernel
        return val, Qn
...
def monge_rotation_kmeans_LR(X, Y, r, lambda_factor=0.5, random_state=0, epsilon=1e-2):
    ...
    A, B = compute_lr_sqeuclidean_factors(X, Y, rescale_cost=False)
